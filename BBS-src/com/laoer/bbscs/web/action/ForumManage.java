...
public class ForumManage extends BaseBoardAction implements RequestBasePathAware, RemoteAddrAware {
	...
	public String m() {
		...
		pages.setFileName(BBSCSUtil.getActionMappingURLWithoutPrefix("forumManage?action=" + this.getAction() + "&bid="
				+ this.getBid()));
		if (this.getTotal() > 0) {
	...
	public String dels() {
		...
		try {
			...
			this.setForwardUrl(this.getBasePath()
					+ BBSCSUtil.getActionMappingURLWithoutPrefix("forumManage?action=m&bid=" + this.getBid() + "&page="
							+ this.getPage()));
			return SUCCESS;
	...
	public String delw() {
		...
		try {
			...
			this.setForwardUrl(this.getBasePath()
					+ BBSCSUtil.getActionMappingURLWithoutPrefix("forumManage?action=w&bid=" + this.getBid() + "&page="
							+ this.getPage()));
			return SUCCESS;
	...
	public String delallw() {
		try {
			...
			this.setForwardUrl(this.getBasePath()
					+ BBSCSUtil.getActionMappingURLWithoutPrefix("forumManage?action=w&bid=" + this.getBid()
							+ "&page=1"));
			return SUCCESS;
	...
	public String auditing() {
		...
		try {
			...
			this.setForwardUrl(this.getBasePath()
					+ BBSCSUtil.getActionMappingURLWithoutPrefix("forumManage?action=a&bid=" + this.getBid() + "&page="
							+ this.getPage()));
			return SUCCESS;
	...
	public String auditingAttach() {
		...
		try {
			...
			this.setForwardUrl(this.getBasePath()
					+ BBSCSUtil.getActionMappingURLWithoutPrefix("forumManage?action=aa&bid=" + this.getBid()
							+ "&page=" + this.getPage()));
			return SUCCESS;
		} catch (BbscsException ex4) {
			this.addActionError(this.getText("error.forummanage.auditing"));
	...
	public String delsnota() {
		...
		try {
			this.getForumService().delPosts(fl, this.getBoard());
			this.setForwardUrl(BBSCSUtil.getActionMappingURLWithoutPrefix("/forumManage?action=a&bid=" + this.getBid()
					+ "&page=" + this.getPage()));
			return SUCCESS;
		} catch (BbscsException ex) {
			this.addActionError(this.getText("error.forummanage.dels"));
	...
	public String delsnotaa() {
		...
		try {
			...
			this.setForwardUrl(BBSCSUtil.getActionMappingURLWithoutPrefix("/forumManage?action=aa&bid=" + this.getBid()
					+ "&page=" + this.getPage()));
			return SUCCESS;
		} catch (BbscsException ex4) {
			this.addActionError(this.getText("error.forummanage.dels"));
