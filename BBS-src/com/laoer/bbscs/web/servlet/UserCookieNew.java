...
public class UserCookieNew {
	...
	// private static final String BBSCS_BID = "BID";
	...
	/*
	 * userCookieArray[0]=postPerNum userCookieArray[1]=forumPerNum
	 * userCookieArray[2]=forumViewMode userCookieArray[3]=lastSendNoteTime
	 * userCookieArray[4]=lastPostTime userCookieArray[5]=editType
	 * userCookieArray[6]=lastActiveTime userCookieArray[7]=addedOnlineTime
	 * userCookieArray[8]=addedOnlineHour userCookieArray[9]=validateCode
	 * userCookieArray[10]=boardPass userCookieArray[11]=bid
		...
		try {
			if (cookies != null && cookies.length > 0) {
				for (int i = 0; i < cookies.length; i++) {
					 ...
					 * StringUtils.isBlank(this.userNameDES)) { if
					 * (sCookie.getName().equalsIgnoreCase("JSESSIONID")) {
					 * this.userName = Constant.GUEST_USERNAME +
					 * sCookie.getValue(); this.userNameDES =
					 * Constant.GUEST_USERNAME + sCookie.getValue(); } }
					 */
				}
			}
		} catch (Exception e) {
	...
	public void addGuestCookies(String gUserName) {
		...
		// this.setUserName(gUserName);

		/*
		 * this.setPostPerNum(20); this.setForumPerNum(10);
		 * this.setTimeZone("GMT+08:00"); this.setForumViewMode(0);
		 * this.setEditType(-1);
		 * this.setLastActiveTime(System.currentTimeMillis());
		 * this.addCookiesArray(-1);
		 */
	...
	public boolean isLoginUser() {
		if (StringUtils.isNotBlank(this.userName) && StringUtils.isNotBlank(this.userNameDES)
